...
struct
  ...
  let next_state c s = match c with
    ...
    | Pop        -> (match s with
                      | []    -> raise Stack.Empty
                      | _::s' -> s')
    | Top        -> if s=[] then raise Stack.Empty else s
    | Clear      -> []
    | Is_empty
    | Length
    | Fold (_,_) -> s
  ...
  let run_cmd c s st = match c with
    ...
    | Pop    ->
      let r = try Some (Stack.pop st)
              with Stack.Empty -> None in
      (match s with
       | []    -> r = None
       | e::_ -> r = Some e)
    | Top    ->
      let r = try Some (Stack.top st)
              with Stack.Empty -> None in
      (match s with
       | []    -> r = None
       | e::_ -> r = Some e)
    ...
    | Fold (f,a) -> Stack.fold (Fn.apply f) a st
